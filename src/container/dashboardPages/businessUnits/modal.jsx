...
const Modal = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchEntities("/api/sites/");
        setSites(data);
      } catch (error) {
  ...
  useEffect(() => {
    if (businessUnitData && Object.keys(businessUnitData).length > 0) {
      setFormData({
        ...
        changed_by: user?.user_data?.id,
        site: businessUnitData?.site?.name || "",
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.name || !formData.description || !formData.site) {
    ...
    try {
      if (businessUnitData && Object.keys(businessUnitData).length > 0) {
        ...
        await createEntity("/api/business_units/", formData);
  ...
  const handleDelete = async () => {
    setLoading(true);
    try {
      await deleteEntity("/api/business_units/", businessUnitData.id);
                ...
                <div className="mb-4">
                  <CustomSelect
                    ...
                    value={siteOptions.find(
                      (option) => option.value === formData.site
                    )}
